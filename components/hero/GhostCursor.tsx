...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  // Real timestamp is set inside the setup effect below, not here —
  // calling performance.now()/Date.now() during render is impure.
  ...
  useEffect(() => {
    ...
    const filmPass = new ShaderPass(FilmGrainShader as unknown as ConstructorParameters<typeof ShaderPass>[0]);
    ...
    // baseVertexShader/fragmentShader are plain string literals redefined
    // every render, and FilmGrainShader/UnpremultiplyPass are memoized on
    // grainIntensity — adding any of them here would re-run this entire
    // WebGL/renderer setup on every render instead of only when the props
    // that actually require rebuilding it change.
  }, [
    trailLength,
    inertia,
    grainIntensity,
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // A <canvas> element's own style is an imperative DOM property, not
    // React state — setting it directly here is the standard way to sync
    // a prop onto an already-created, ref-held DOM node.
    if (mixBlendMode) {
